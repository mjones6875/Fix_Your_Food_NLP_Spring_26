...
Original file is located at
    https://colab.research.google.com/drive/1VMfKhBO8EEdJ_67GQ2X4T6rMCq2lRuak
"""
...
def load_reviews(file_path, star_col='StarRating', text_col='Comment',
    ...
    df[star_col] = df[star_col].astype(int)
...
if os.path.exists('glove.6B.100d.txt'):
    print('glove.6B.100d.txt downloaded and extracted successfully!')
else:
    print('Error: glove.6B.100d.txt not found after download and extraction.')

def main():
    FILE_PATH = '/content/mcdonaldsfoodreviews.csv'
    ...
    print(f"Loaded {len(df)} reviews. Example:")
    print(df.head())
    ...
    embedding_matrix = load_glove_embeddings(GLOVE_PATH, tokenizer.word_index,
                                             EMBEDDING_DIM)
    ...
    return df, X_train, y_train, X_val, y_val, X_test, y_test, model, history

if __name__ == '__main__':
    df, X_train, y_train, X_val, y_val, X_test, y_test, model, history = main()
...
for i, label in enumerate(class_labels):
    ...
    if np.sum(mask) > 0:
        ...
        print(f"{label}: mean confidence = {np.mean(probs):.4f}, "
              f"std = {np.std(probs):.4f}, min = {np.min(probs):.4f}")